...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  arr.forEach(keyHouse =>{
    houses.push(keyHouse['house']);
  })
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(characteValue => {
    if (characteValue.name === character){
      if(Object.values(characteValue)[2].length > 0){
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(characteValue => {
    if (characteValue.name === character){
      if(Object.entries(characteValue.children).length > 0){
        childrenPresent = true;
      }
    }
  })
